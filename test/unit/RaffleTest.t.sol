...
contract RaffleTest is Test{
    ...
    function testRaffleRevertsWhenRaffleNotOpen() public raffleEnteredAndTimePassed{
    ...
    function testFulfillRandomWordsPicksAWinnerResetsAndSendsMoney() public raffleEnteredAndTimePassed skipFork{
        ...
        for(uint256 i = startingIndex ; i <= totalPlayers ; i++){
            ...
            hoax(player , STARTING_USER_BALANCE);         // hoax Does vm.deal and vm.prank both in same step. 
            raffle.enterRaffle{value : entranceFee}();
        }
        uint256 prize = entranceFee*(totalPlayers + 1);
        vm.recordLogs();
        raffle.performUpkeep();
        Vm.Log[] memory entries = vm.getRecordedLogs();
        bytes32 requestId = entries[1].topics[1];
